package com.xy.bussiness.yahoo.mapper;
...
@Mapper
public interface YahooItemMapper extends BaseMapper<YahooItemRecord> {

    @Select("<script>" +
            "SELECT * FROM (" +
            "SELECT *, ROW_NUMBER() OVER (PARTITION BY search_condition_id ORDER BY COALESCE(create_date, update_date) DESC) AS rn " +
            "FROM yahoo_item_record WHERE search_condition_id IN " +
            "<foreach collection='conditionIds' item='id' open='(' separator=',' close=')'>#{id}</foreach>" +
            ") ranked WHERE ranked.rn &lt;= #{limit}" +
            "</script>")
    List<YahooItemRecord> getLatestItemsByConditionIds(@Param("conditionIds") List<Integer> conditionIds,
